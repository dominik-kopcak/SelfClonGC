...
## Calcualate ID using inbreedR
	for(vcf_file in vcf_file_list_selfing){
		...
		Replicates_selfing_vec <- append(gsub("replicate", "", strsplit(vcf_file, split = "_")[[1]][7]), Replicates_selfing_vec)
		Rate_selfing_vec <- append(as.numeric(gsub("selfing", "", strsplit(vcf_file, split = "_")[[1]][6])), Rate_selfing_vec)
	}
...
write.csv(ID_df_selfing, file="Data_ID_selfing.csv")
