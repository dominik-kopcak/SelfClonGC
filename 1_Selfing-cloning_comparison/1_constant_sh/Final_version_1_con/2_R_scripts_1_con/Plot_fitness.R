...
for (h_coefficient in h_values){
	...
	for (rate in Uniparenting_rates){
		...
		Plot_mean_selfing <- Plot_mean_selfing +
			geom_line(data=fitness_selfing_subset, aes(x=2*Time, y=Expected_mean_fitness, colour = Uniparenting_rate))+
			geom_ribbon(data=fitness_selfing_subset, aes(x=2*Time, ymin=(Expected_mean_fitness - 1.96*SD_mean_fitness/sqrt(N_replicates)), ymax=(Expected_mean_fitness + 1.96*SD_mean_fitness/sqrt(N_replicates)), fill = Uniparenting_rate), alpha = 0.5)
			
		Plot_mean_cloning <- Plot_mean_cloning +
			geom_line(data=fitness_cloning_subset, aes(x=2*Time, y=Expected_mean_fitness, colour = Uniparenting_rate))+
			geom_ribbon(data=fitness_cloning_subset, aes(x=2*Time, ymin=(Expected_mean_fitness - 1.96*SD_mean_fitness/sqrt(N_replicates)), ymax=(Expected_mean_fitness + 1.96*SD_mean_fitness/sqrt(N_replicates)), fill = Uniparenting_rate), alpha = 0.5)
				
		Plot_variance_selfing <- Plot_variance_selfing +
			geom_line(data=fitness_selfing_subset, aes(x=2*Time, y=Expected_variance_fitness, colour = Uniparenting_rate))+
			geom_ribbon(data=fitness_selfing_subset, aes(x=2*Time, ymin=(Expected_variance_fitness - 1.96*SD_variance_fitness/sqrt(N_replicates)), ymax=(Expected_variance_fitness + 1.96*SD_variance_fitness/sqrt(N_replicates)), fill = Uniparenting_rate), alpha = 0.5)
			
		Plot_variance_cloning <- Plot_variance_cloning +
			geom_line(data=fitness_cloning_subset, aes(x=2*Time, y=Expected_variance_fitness, colour = Uniparenting_rate))+
			geom_ribbon(data=fitness_cloning_subset, aes(x=2*Time, ymin=(Expected_variance_fitness - 1.96*SD_variance_fitness/sqrt(N_replicates)), ymax=(Expected_variance_fitness + 1.96*SD_variance_fitness/sqrt(N_replicates)), fill = Uniparenting_rate), alpha = 0.5)
...
plot_mean_selfing_name <- paste("Figure_mean_fitness_selfing_", h_coefficient, "_full_60000.png", sep="")
ggsave(filename = plot_mean_selfing_name,
	plot = Plot_mean_selfing,
	dpi = 600)

plot_mean_cloning_name <- paste("Figure_mean_fitness_cloning_", h_coefficient, "_full_60000.png", sep="")
ggsave(filename = plot_mean_cloning_name,
	plot = Plot_mean_cloning,
	dpi = 600)

plot_variance_selfing_name <- paste("Figure_variance_fitness_selfing_", h_coefficient, "_full_60000.png", sep="")
ggsave(filename = plot_variance_selfing_name,
	plot = Plot_variance_selfing,
	dpi = 600)

plot_variance_cloning_name <- paste("Figure_variance_fitness_cloning_", h_coefficient, "_full_60000.png", sep="")
