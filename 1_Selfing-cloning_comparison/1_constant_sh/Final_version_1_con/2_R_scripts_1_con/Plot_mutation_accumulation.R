...
Cloning_rel_max <- filename_rel_cloning$Expected_relative_homozygosity+1.96*filename_rel_cloning$SD_relative_homozygosity/sqrt(replicate_number)
Cloning_rel_min <- filename_rel_cloning$Expected_relative_homozygosity-1.96*filename_rel_cloning$SD_relative_homozygosity/sqrt(replicate_number)

Plot_recessive <- ggplot()+
	geom_point(data = filename_rec_selfing, aes(x = Uniparenting_rate, y = Expected_recessive_load, colour="Selfing"), size=3, shape=15)+
	...
	geom_point(data=filename_rec_cloning, aes(x=Uniparenting_rate, y=Expected_recessive_load, colour="Asexuality"), size=3, shape=15)+
...
Plot_additive_log <- ggplot()+
	geom_point(data=filename_add_selfing, aes(x=Uniparenting_rate, y=Expected_additive_load), colour="darkolivegreen2", size=3, shape=15)+
	geom_linerange(data=filename_add_selfing, 
			aes(x=Uniparenting_rate, 
			    ymax=Selfing_add_max, 
			    ymin=Selfing_add_min), color="darkolivegreen2", size=1)+
	
	geom_point(data=filename_add_cloning, aes(x=Uniparenting_rate, y=Expected_additive_load), colour="darkolivegreen", size=3, shape=15)+
	geom_linerange(data=filename_add_cloning, 
			aes(x=Uniparenting_rate, 
...
Plot_3 <- Plot_additive_log/Plot_recessive

plot_name <- paste("Figure_Mutation_accumulation_selfing_vs_cloning_", h_coefficient, "_full_60000.png",sep="")
ggsave(filename=plot_name,
	plot=Plot,
	dpi=600)

plot_name_2 <- paste("Figure_Mutation_accumulation_selfing_vs_cloning_", h_coefficient, "v2_full_60000.png",sep="")
ggsave(filename=plot_name_2,
	plot=Plot_2,
	dpi=600)

plot_name_log <- paste("Figure_Mutation_accumulation_selfing_vs_cloning_", h_coefficient, "v3_full_60000.png",sep="")
ggsave(filename=plot_name_log,
	plot=Plot_3,
	dpi=600)
	
plot_name_rel <- paste("Figure_Mutation_accumulation_selfing_vs_cloning_", h_coefficient, "relative_homozygosity_full_60000.png",sep="")
