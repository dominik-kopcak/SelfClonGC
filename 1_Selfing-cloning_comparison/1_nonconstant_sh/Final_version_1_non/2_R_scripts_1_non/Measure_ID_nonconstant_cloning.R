...
#vcf_file_list_cloning <- vcf_file_list_cloning[grepl("replicate100", vcf_file_list_cloning)]
...
for(vcf_file in vcf_file_list_cloning){
	...
	Replicates_cloning_vec <- append(gsub("Replicate", "", strsplit(vcf_file, split = "_")[[1]][5]), Replicates_cloning_vec)
	Rate_cloning_vec <- append(as.numeric(gsub("cloning", "", strsplit(vcf_file, split = "_")[[1]][4])), Rate_cloning_vec)
}
...
write.csv(ID_df_cloning, file="Data_ID_nonconstant_cloning.csv")
