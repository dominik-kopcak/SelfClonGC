...
CLON=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/1_Selfing-cloning_comparison/nonconstant_sh/parameters_partial_2.txt | awk '{print $1}')
REP=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/1_Selfing-cloning_comparison/nonconstant_sh/parameters_partial_2.txt | awk '{print $2}')

NAME_cloning=cloning${CLON}_Replicate${REP}_time60000

slim -d Clon_rate=$CLON -d "File_path_i='/scratch/dkopcak/1_Selfing-cloning_comparison/X_Sampled_individuals_$NAME_cloning.vcf'" -d "File_path_f='/scratch/dkopcak/1_Selfing-cloning_comparison/X_Fixed_mutations_$NAME_cloning.csv'" -d "Fitness_mean_file_name='/scratch/dkopcak/1_Selfing-cloning_comparison/Wm_Mean_fitness_$NAME_cloning.csv'" -d "Fitness_variance_file_name='/scratch/dkopcak/1_Selfing-cloning_comparison/Wv_Variance_fitness_$NAME_cloning.csv'" Mutation_accumulation_nonconstant_sh_time60.slim
