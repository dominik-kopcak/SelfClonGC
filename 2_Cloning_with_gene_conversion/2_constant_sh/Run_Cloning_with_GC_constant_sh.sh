...
#$ -V
#$ -cwd
#$ -t 1-40		# Run command for each line of parameter file
...
SEL=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/2_Cloning_with_gene_conversion/constant_sh/parameters_all_2.txt | awk '{print $1}')
DOM=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/2_Cloning_with_gene_conversion/constant_sh/parameters_all_2.txt | awk '{print $2}')
GAMMA=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/2_Cloning_with_gene_conversion/constant_sh/parameters_all_2.txt | awk '{print $3}')
LAMBDA=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/2_Cloning_with_gene_conversion/constant_sh/parameters_all_2.txt | awk '{print $4}')
REP=$(sed -n ${SGE_TASK_ID}p /data/hartfield/asex_load/scripts/2_Cloning_with_gene_conversion/constant_sh/parameters_all_2.txt | awk '{print $5}')
