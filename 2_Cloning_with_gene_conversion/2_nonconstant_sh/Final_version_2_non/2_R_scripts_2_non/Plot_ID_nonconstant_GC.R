...
ID_df_nonconstant_GC <- read.csv("Data_ID_nonconstant_GC.csv")
ID_df_nonconstant_GC$Rep <- as.factor(ID_df_nonconstant_GC$Rep)

replicate_number = length(unique(ID_df_nonconstant_GC$Rep))

ID_df_nonconstant_GC_sum <- ID_df_nonconstant_GC %>%
	group_by(Rate) %>%
...
print(str(ID_df_nonconstant_GC))

g2_plot <- ggplot() + 
	geom_linerange(data = ID_df_nonconstant_GC, aes(x=Rate*1000, ymin=CI_lower, ymax=CI_upper), size = 2, alpha = 0.1) + 
	geom_point(data = ID_df_nonconstant_GC_sum, aes(x=Rate*1000, y=Expected_g2), size = 4) + 
	geom_errorbar(data = ID_df_nonconstant_GC_sum, aes(x=Rate*1000, ymin=Expected_g2-1.96*sd_g2/sqrt(replicate_number), ymax=Expected_g2+1.96*sd_g2/sqrt(replicate_number))) +
    scale_color_brewer(palette = "RdBu") + 
	labs(x=expression("Mean GC rate per site (" %*% "4 )"),y=expression(paste("Identity disequilibrium (",g[2],")")), 
		title="ID for asexual populations\nwith gene conversion\nand variable fitness effects",
...
ggsave(filename = "Plot_ID_g2_nonconstant_GC.png",
	plot = g2_plot,
	dpi = 300,
	width=12,
	height=12)
