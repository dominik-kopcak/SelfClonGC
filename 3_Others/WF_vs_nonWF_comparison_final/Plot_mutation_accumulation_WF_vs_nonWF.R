...
Cloning_GC_recessive_load <- read.csv("Data_GC_recessive_load_vXII_full_60000.csv")
Cloning_GC_additive_load <- read.csv("Data_GC_additive_load_vXII_full_60000.csv")
...
Cloning_recessive_load <- read.csv("Data_cloning_recessive_load_full_60000.csv")
Cloning_additive_load <- read.csv("Data_cloning_additive_load_full_60000.csv")
...
Plot_recessive <- ggplot()+
	...
	labs(y="Homozygous\ngenotypes",
	     x="Rate of cloning or mean GC per site",
	...
	scale_colour_manual(values = colours, labels=c("Partial or complete asex.                 ", 
	...
	scale_x_discrete(labels = c("0.95", "0.99", "1", expression(4 %*% 10 ^ -5), 
							 expression(4 %*% 10 ^ -6), 
							 expression(4 %*% 10 ^ -7), 
							 expression(4 %*% 10 ^ -8)))
