...
setwd("/mnt/loki/hartfield/AsexMuts/scripts/SelfClonGC/5_small_pop_size/3_data_newpop2/")
...
# Calculating CI for relative values using bootstrapping
rhSD <- tibble(GC_rate=double(), SD_relative_homozygosity_min=double(), SD_relative_homozygosity_max=double())

for(g in gamma_values)
{
	gRH <- Relative_homozygosity_GC %>% filter(GC_rate == g) %>% select(Relative_homo_GC) %>% .[,1]
	BSval <- vector(mode="double",length=1000)
	for(i in 1:1000){
		BSval[i] <- mean(sample(gRH,replace=T))
	}
	rhSD <- rhSD %>% add_row(GC_rate=g, SD_relative_homozygosity_min=quantile(BSval,c(0.025,0.975))[1], SD_relative_homozygosity_max=quantile(BSval,c(0.025,0.975))[2])
}
Relative_homozygosity_stat_GC <- Relative_homozygosity_stat_GC %>% inner_join(rhSD)
	
write.csv(Additive_load_stat_GC, file="../4_plots/Data_GC_additive_load_vXII_full_60000_smallpop.csv")
write.csv(Recessive_load_stat_GC, file="../4_plots/Data_GC_recessive_load_vXII_full_60000_smallpop.csv")
write.csv(Relative_homozygosity_stat_GC, file="../4_plots/Data_GC_relative_homozygosity_vXII_full_60000_smallpop.csv")

write.csv(Additive_load_GC, file="../4_plots/Data_GC_additive_load_vXII_full_60000_raw_smallpop.csv")
write.csv(Recessive_load_GC, file="../4_plots/Data_GC_recessive_load_vXII_full_60000_raw_smallpop.csv")
write.csv(Relative_homozygosity_GC, file="../4_plots/Data_GC_relative_homozygosity_vXII_full_60000_raw_smallpop.csv")
