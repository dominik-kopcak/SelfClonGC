...
#SBATCH --array=1-140%32      	        		 	  	  # Array tasks
...
slim -d s_coeff=$SEL -d h_coeff=$DOM -d init_GC_rate=$GAMMA -d lambda=$LAMBDA -d GC_rate_SD=0 -d "File_path_i='X_Sampled_individuals_$NAME.vcf'" -d "File_path_f='X_Fixed_mutations_$NAME.csv'" -d "Fitness_mean_file_name='Wm_Mean_fitness_$NAME.csv'" -d "Fitness_variance_file_name='Wv_Variance_fitness_$NAME.csv'" -d "GC_mean_file_name='GC_mean_values_$NAME.csv'" -d "GC_var_file_name='GC_var_values_$NAME.csv'" AsexGC_w_modifier.slim
mv Cycle_*_X_Sampled_individuals_$NAME.vcf Cycle_*_X_Fixed_mutations_$NAME.csv Wm_Mean_fitness_$NAME.csv Wv_Variance_fitness_$NAME.csv GC_mean_values_$NAME.csv GC_var_values_$NAME.csv ../2_outputs/
