...
Mean_GC <-rep(c(0), each=N_s_values*N_h_values*N_gamma_values*N_lambda_values*Max_time*N_replicates)
...
Fitness_GC <- data.frame(Selection_coefficient, Dominance_coefficient, GC_rate, Mean_tract_length, Replicate, Time, Mean_fitness_GC, Variance_fitness_GC, Mean_GC)
...
Fitness_GC_stat <- Fitness_GC %>%
	...
	summarize(Expected_mean_fitness = mean(Mean_fitness_GC), Expected_variance_fitness = mean(Variance_fitness_GC), Expected_GC = mean(Mean_GC),
		  SD_mean_fitness = sd(Mean_fitness_GC), SD_variance_fitness = sd(Variance_fitness_GC), SD_GC = sd(Mean_GC), .groups = "rowwise")
