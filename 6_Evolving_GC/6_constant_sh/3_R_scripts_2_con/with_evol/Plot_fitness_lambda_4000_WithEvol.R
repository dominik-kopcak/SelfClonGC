...
for (h_coefficient in h_values){
	...
	for (rate in unique(Fitness_GC$GC_rate)){
		...
		Plot_mean_GC <- Plot_mean_GC +
			geom_line(data=fitness_GC_subset, aes(x=Time, y=Expected_mean_fitness, colour = GC_rate, linetype = GC_var))+
			geom_ribbon(data=fitness_GC_subset, aes(x=Time, ymin=CI_fit_L, ymax=CI_fit_U, fill = GC_rate, linetype = GC_var), alpha = 0.5)
			
		Plot_variance_GC <- Plot_variance_GC +
			geom_line(data=fitness_GC_subset, aes(x=Time, y=Expected_variance_fitness, colour = GC_rate, linetype = GC_var))+
			geom_ribbon(data=fitness_GC_subset, aes(x=Time, ymin=CI_var_L, ymax=CI_var_U, fill = GC_rate, linetype = GC_var), alpha = 0.5)
			
		Plot_evol_GC <- Plot_evol_GC +
			geom_line(data=fitness_GC_subset, aes(x=Time, y=Expected_GC, colour = GC_rate, linetype = GC_var))+
			geom_ribbon(data=fitness_GC_subset, aes(x=Time, ymin=CI_GC_L, ymax= CI_GC_U, fill = GC_rate, linetype = GC_var), alpha = 0.5)
