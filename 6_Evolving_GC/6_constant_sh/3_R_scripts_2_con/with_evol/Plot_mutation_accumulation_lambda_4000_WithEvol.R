...
Plot_recessive <- ggplot()+
 	geom_point(data=filename_rec_GC, aes(x=Cycle, y=Expected_recessive_load, colour=GC_rate), size=3, shape=15)+
	geom_linerange(data=filename_rec_GC, 
			aes(x=Cycle, 
			    ymax=GC_rec_max, 
			    ymin=GC_rec_min,colour=GC_rate),size=1)+
		scale_colour_viridis_d(breaks=unique(filename_rec_GC$GC_rate), labels=c(expression(4 %*% 10 ^ -2), expression(4 %*% 10 ^ -4), expression(4 %*% 10 ^ -7)),direction=-1) + 
...
Plot_additive <- ggplot()+
	geom_point(data=filename_add_GC, aes(x=Cycle, y=Expected_additive_load/1000, colour=GC_rate), size=3, shape=15)+
...
Plot_relative <- ggplot()+
 	geom_point(data=filename_rel_GC, aes(x=Cycle, y=Expected_relative_homozygosity, colour=GC_rate), size=3, shape=15)+
